...
- **Balanced for the use case** — the task defines what "balanced" means; a pediatric-cancer model is curated to children, not adults
- **Kept balanced over time** — **regularly audit** for creeping bias and correct it (rebalancing/augmentation mechanics in [§1.6](../domain-1/#16-the-ml-lifecycle-end-to-end))

## 4.3 Bias & Variance

*The statistical pair — distinct from fairness-bias.*

Two failure modes, each driven by one error source and showing a distinct error signature:

<ComparisonTable
  caption="Bias vs. variance: the two failure modes"
  columns={['Fit', 'Error source', 'Training error', 'Validation error', 'Typical cause']}
  rows={[
    ['Underfitting', 'High <strong>bias</strong> (too simple, misses patterns)', 'High', 'High', 'Model lacks capacity for the real pattern'],
    ['Overfitting', 'High <strong>variance</strong> (over-sensitive to noise)', 'Low (→0)', 'High', 'Excess complexity, or a too-small training/fine-tuning set'],
    ['Balanced (goal)', 'Low bias + low variance', 'Low', 'Low', 'Well-matched capacity: the trade-off sweet spot'],
  ]}
/>

<Callout>
  Two axes, one dial. <strong>High bias → underfitting</strong> (too simple: poor on training <em>and</em> new data). <strong>High variance → overfitting</strong> (too complex: great on training, poor on new data). Turning complexity up trades bias for variance, so you tune to the sweet spot.
</Callout>
...
| **SageMaker Ground Truth** | **Human data labeling** for training data; for RLHF, humans rank/compare model responses to build the **reward model**. Contrast **A2I** = humans review *predictions* at inference |
...
<Callout>
  <strong>Guardrails</strong> = filter the output · <strong>Clarify</strong> = bias · <strong>Model Monitor</strong> = drift · <strong>A2I</strong> = humans · <strong>Model Cards</strong> = your paperwork · <strong>Service Cards</strong> = AWS's paperwork
</Callout>

**AI Service Cards** (distinct from Model Cards) — AWS-published documentation for each AWS AI service: intended use cases and limitations, responsible-AI design choices, and deployment/performance best practices. The split to remember: **Model Cards = docs you write for *your* model; Service Cards = docs AWS publishes for *its* services.**
